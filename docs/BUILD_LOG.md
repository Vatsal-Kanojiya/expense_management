...
| 36 | **Line items vanish when the expense form has an error** | Leave a required field such as `note` blank with line items filled in, and the re-rendered page has none of them. The POST carries every item; the server discards them. Real data loss on the most-used form. Root cause: `ItemFormSetMixin` builds a bound formset only in `form_valid`, and Django's `form_invalid` path falls back to an unbound one in `get_context_data`. The comment beside that fallback claims the opposite, which is why review missed it. Introduced in phase 8 (session 9) | **Handoff T8** in `docs/HANDOFF_PLAN.md`. A `form_invalid` that re-binds the formset to the POST and validates it, so both error sets show in one round trip. No persistence — the POST is already the state |
| 37 | Required fields are not marked | Nothing shows a field is mandatory until submitting fails | **Handoff T9.** One `:has()` CSS rule keyed on the `required` attribute Django already renders, covering every `as_p` form including Django's own auth forms, plus a manual marker on the two required line-item columns. `misc_note` is deliberately unstarred because it is only conditionally required |
